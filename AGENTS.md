...
2. **Minimal default schemas.** List commands return only 3–4 useful fields by default, normally an identifier plus title/name and state/status. Do not put `body`, comments, descriptions, URLs, nested users, or other verbose fields in list defaults — except the documented `Comment` entity exception ([ADR 0019](docs/adr/0019-include-comment-body-in-default-projections.md)), where `body` is the primary payload. Expose additional fields through `--fields`.
